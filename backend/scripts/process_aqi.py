...
# 监测站名称映射：CSV列名 -> 目标模型中的站名
# STATION_MAPPING = {
#     "东四": "东四", "天坛": "天坛", "官园": "官园", "万寿西宫": "万寿西宫",
#     "奥体中心": "奥体中心", "农展馆": "农展馆", "万柳": "海淀万柳",
#     "顺义": "顺义新城", "怀柔": "怀柔镇", "昌平": "昌平镇",
#     "延庆": "延庆夏都", "密云": "密云新城", "门头沟": "门头沟三家店",
#     "云岗": "丰台云岗", "通州": "通州东关", "房山": "房山燕山",
#     "大兴": "大兴旧宫", "丰台花园": "丰台小屯", # 假设丰台花园是丰台小屯
#     "古城": "八角北路9号", "密云水库": "洳河巡河路", "定陵": "定陵",
# }
...
STATION_MAPPING = {
    ...
    "朝阳奥体中心": "奥体中心",
    "朝阳农展馆": "农展馆",
    "海淀万柳": "海淀万柳",  # 2021年可能也有这个名字，或者与2020年相同
    "丰台小屯": "丰台小屯",  # 2021年可能也有这个名字
    "丰台云岗": "丰台云岗",  # 2021年可能也有这个名字
    "石景山古城": "八角北路9号",  # 2021年古城的新名称
    "昌平镇": "昌平镇",  # 2021年可能也有这个名字
    "定陵(对照点)": "定陵",  # 2021年定陵的新名称
    "延庆夏都": "延庆夏都",  # 2021年可能也有这个名字
    "延庆石河营": "延庆石河营",  # 2021年可能有这个名称
    "怀柔镇": "怀柔镇",  # 2021年可能也有这个名字
    "怀柔新城": "怀柔新城",  # 2021年可能有这个名称
    "密云镇": "密云镇",  # 2021年可能有这个名称
    "密云新城": "密云新城",  # 2021年可能也有这个名字
    "顺义新城": "顺义新城",  # 2021年可能也有这个名字
    "通州东关": "通州东关",  # 2021年可能也有这个名字
    "大兴旧宫": "大兴旧宫",  # 2021年可能也有这个名字
    "门头沟三家店": "门头沟三家店",  # 2021年可能也有这个名字
    "房山燕山": "房山燕山",  # 2021年可能也有这个名字
}

# CSV文件中我们需要保留的站点的列名（即 STATION_MAPPING 的键）
SELECTED_CSV_STATION_COLUMNS = list(STATION_MAPPING.keys())

# 根据您提供的图片，将清洗后的标准站点名称映射到对应的ID
...
def process_daily_data(current_date):
    ...
    merged_df = pd.DataFrame()  # 确保在所有路径中都已初始化

    any_file_found_and_read = False  # 标记当天是否有文件被成功读取
    ...
    if os.path.exists(file_all_path):
        try:
            ...
            print(f"  Warning: _all file for {date_str} is empty or only contains headers.")
        except Exception as e:
            print(f"  Error reading/processing _all file for {date_str}: {e}")
    else:
    ...
    if os.path.exists(file_extra_path):
        try:
            ...
            print(f"  Warning: _extra file for {date_str} is empty or only contains headers.")
        except Exception as e:
            print(f"  Error reading/processing _extra file for {date_str}: {e}")
    else:
    ...
    if not df_all_processed.empty and not df_extra_processed.empty:
        merged_df = pd.merge(df_all_processed, df_extra_processed,
                             ...
                             how='outer')
        print(
            f"  Merged _all ({len(df_all_processed)} records) and _extra ({len(df_extra_processed)} records) into {len(merged_df)} records.")
    elif not df_all_processed.empty:
        merged_df = df_all_processed
        print(f"  Only _all data available for merging: {len(merged_df)} records.")
    elif not df_extra_processed.empty:
        merged_df = df_extra_processed
        print(f"  Only _extra data available for merging: {len(merged_df)} records.")
    else:
        # >>> MODIFIED: 更清晰地表示为什么返回空DataFrame <<<
        if any_file_found_and_read:
            print(f"  No valid records could be generated from either file for merging, despite files being present.")
        else:
            print(f"  No valid records could be generated as no relevant files were found for this date.")
        print(f"--- Finished processing {date_str}: 0 total records from this date ---")
        return pd.DataFrame()  # 如果两个处理后的DataFrame都为空，则返回空DataFrame
...
if __name__ == "__main__":
    ...
    if all_yearly_records:
        ...
        print(f"\nTotal records collected for the year: {len(final_yearly_df)}")
        final_yearly_df.to_csv(OUTPUT_FILE, index=False, encoding='utf-8')
        print(f"Data successfully exported to {OUTPUT_FILE}")
    else:
        print("No data processed for the specified date range.")